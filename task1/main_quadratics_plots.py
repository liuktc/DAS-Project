...
def run_experiment(num_agents, vars_dim, out_dir, seed):
    ...
    local_loss, global_loss, optimal_z, z0 = setup_quadratic_problem(num_agents=num_agents, vars_dim=vars_dim, seed=seed)
    ...
    for graph_type in GRAPH_TYPES:
        ...
        estimated_cost[graph_type] = sum( local_loss[i](history_z[graph_type][-1, i]) for i in range(num_agents) )
        
    #Print final cost
    print(f"Cost {'optimal':<15}: {optimal_cost:.10f}")
    for graph_type in GRAPH_TYPES:
        print(f"Cost {graph_type:<15}: {estimated_cost[graph_type]:.10f} | Diff: {abs(estimated_cost[graph_type] - optimal_cost):.10f}")
...
for num_agents in NUM_AGENTS_LIST:
    for vars_dim in NUM_VARS_LIST:
        run_experiment(num_agents, vars_dim, f"./task_1.1_{num_agents}_{vars_dim}",SEED)
