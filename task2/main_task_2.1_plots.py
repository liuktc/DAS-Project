...
def run_experiments(NUM_ROBOTS, VAR_DIMS, GAMMAS, SEED, out_dir):
    ...
    for graph_type in GRAPH_TYPES:
        ...
        z_history[graph_type], s_history[graph_type], v_history[graph_type] = (
            gradient_tracking_aggregative(
    ...
    for graph_type in GRAPH_TYPES:
        grad_cost_history = np.array(
            [
                ...
                for z, v, s in zip(
                    z_history[graph_type], v_history[graph_type], s_history[graph_type]
                )
            ]
        )
        print(grad_cost_history.shape)
...
for num_robots in NUM_ROBOTS:
    for gamma in GAMMAS:
        ...
        run_experiments(
            ...
            GAMMAS=gammas,
            SEED=SEED,
            out_dir=f"./task_2.1/{num_robots}_{gamma}",
        )
